...
- points 3개는 서로 다른 사실을 담아야 한다. 같은 문장을 분기·연도·수치 등 일부 단어만 바꿔 반복하지 않는다. 원문에 유사한 사실이 여러 번(예: 분기별 수치) 나오더라도 그중 가장 중요한 것 하나만 고르고, 나머지 point 자리는 원문에 있는 다른 사실로 채운다.
...
- points 3개가 서로 다른 사실을 담고 있는가? 숫자나 단어만 바꿔 같은 문장을 반복한 point는 없는가?
