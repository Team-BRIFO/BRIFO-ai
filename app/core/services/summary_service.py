...
def _build_prompt(request: CardNewsGenerateRequest) -> str:
    ...
    return (
        ...
        "작성한다. 후보가 3개보다 많으면 이 기사를 이해하는 데 가장 중요한 용어부터 우선 선택한다. "
        f"단, 다음 용어는 최근 14일 내 이미 출제되었으므로 절대 다시 고르지 않는다: {exclude_terms}. "
        "이 용어들을 제외하고 남은 용어 중에서 고른다. 제외 대상 외에 다른 용어 후보가 없다면 "
        "차라리 2개만 작성한다.\n"
