...
- contentText: 모바일 화면에서 읽으므로 하나의 문단으로 몰아 쓰지 않는다. 2~3개의 문단으로 나누고
  문단 사이는 빈 줄(줄바꿈 두 번, \\n\\n)로 구분한다. 문단 구성 예: 1) 핵심 사실과 근거 2) 해석·전망
  또는 배경지식·리스크 (PRO는 필요시 3) 최종 판단까지 세 문단). 각 문단은 2~3문장 이내로 짧게 유지한다.
  글머리 기호가 필요하면 "- "가 아니라 "• "만 사용하고, 굵게(**)·제목(#) 등 마크다운 문법은 쓰지 않는다.
...

{
  "headline": "한 줄 결론",
  "summary": "짧은 요약",
  "contentText": "첫 번째 문단(핵심 사실과 근거).\\n\\n두 번째 문단(해석과 전망, 또는 배경지식과 리스크).",
  "oneLiner": "마지막 한마디",
...
- contentText가 빈 줄(\\n\\n)로 구분된 2~3개의 문단으로 나뉘었는가? 마크다운 문법(굵게 **, 제목 #, 글머리 기호 "- " 등)을 쓰지 않았는가? (TANKER의 하방 낙폭 "-12%" 같은 음수 부호 표기는 예외다.)
