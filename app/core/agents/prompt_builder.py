...
_COMMON_RULES = """\
- 카드뉴스의 사실관계와 수치는 변경하지 않고 정확히 반영한다. (요약 과정에서 문장을 압축하는 것은 가능하지만, 수치·사실 자체를 바꾸거나 지어내지 않는다.)
- 배경 지식(과거 유사 사례, 시장 통념, 업계 일반 지식 등)을 최소 1개 포함해 카드뉴스 요약을 넘어서는 분석을 제공한다. 단, 구체적인 기업명·수치·사건은 카드뉴스에 있는 것만 사용하고 새로 지어내지 않는다.
...
def build_briefing_prompt(
    ...
    try:
        level_effect = profile["level_effects"][level_range]
...
반드시 위 <news_cards>의 내용을 기반으로 새로 작성한다.
<example>
...
def _format_decision(decision: RecentDecision) -> str:
    if decision.is_correct is None:
        result = "정산 대기 중"
    elif decision.is_correct:
        result = "적중"
    else:
        result = "실패"
    return (
        f"- {escape(str(decision.stock_name))}: {decision.direction} 예측"
        f" (확신도 {decision.confidence}, {result})"
    )
